...
//수정된 c++구조체: 회전값 추가, 패딩 조정
struct ConstantData {   
	XMFLOAT2 offset;    //8바이트
    float rotation;     //4바이트
	//4바이트 -> 합쳐서 16바이트! (이거 안 하면 출력 안 됨)
	float padding;      //16바이트 단위를 맞추기 위한 빈 공간 (4바이트)
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    const char* shaderSource = R"(
        cbuffer MoveBuffer : register(b0)
        {
            float2 g_Offset;
            float g_Rotation;
            float g_Padding;        // 4바이트 추가! (순서와 크기를 C++과 똑같이 맞춤)
        };
        ...
        PS_INPUT VS_Main(VS_INPUT input) 
        {
            ...
            rotPos.x = input.pos.x * c - input.pos.y * s;       //rotPos.x = input.pos.x * c + input.pos.y * s;  -> rotPos.x = input.pos.x * c - input.pos.y * s; 
