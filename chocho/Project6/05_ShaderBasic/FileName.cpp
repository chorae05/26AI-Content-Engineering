...
// --- [해상도 및 리소스 재구성 함수] ---
// 교수님 6주차 핵심 강조: "해상도 바꿀 땐 반드시 도화지를 반납하고 새로 받아라"
void RebuildVideoResources(HWND hWnd) {
    if (!g_pSwapChain) return;      //물어보고(Check) 지우는 습관을 들여라!"

    // 1. [해제 단계] 기존 렌더 타겟 뷰 해제 
    // 교수님 강조: "도화지를 쥐고 있으면 크기를 못 바꾼다. 반드시 놓아줘라!"
    // 1. 기존 렌더 타겟 뷰 해제 (안 하면 ResizeBuffers 실패함)
    if (g_pRenderTargetView) {
        g_pRenderTargetView->Release();    // 현재 사용 중인 도화지를 그래픽 카드에 돌려줘. 
        g_pRenderTargetView = nullptr;      // 안전하게 비워두기
    }

    // 2. [변경 단계] 백버퍼 크기 재설정(물리적인 픽셀 개수를 늘리거나 줄임)
    // 스왑체인 내부의 도화지 원판 크기를 g_Config에 적힌 새 수치로 바꿔.
    g_pSwapChain->ResizeBuffers(0, g_Config.Width, g_Config.Height, DXGI_FORMAT_UNKNOWN, 0);

    // 3. 새 백버퍼로부터 렌더 타겟 뷰 다시 생성
    ID3D11Texture2D* pBackBuffer = nullptr;
    // (0번 인덱스 버퍼를, 2D 텍스처 타입으로, pBackBuffer 주소에 담아줘)
    g_pSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&pBackBuffer);
    // 이 텍스처(pBackBuffer)를 도화지 표면(g_pRenderTargetView)으로 쓰겠다고 확정!
    g_pd3dDevice->CreateRenderTargetView(pBackBuffer, nullptr, &g_pRenderTargetView);
    // 가져온 원본 텍스처는 뷰를 만들었으니 이제 해제해도 돼. (중요!)
    pBackBuffer->Release();
    ...
    if (!g_Config.IsFullscreen) {
        ...
        // 우리가 정한 순수 그림 영역(800x600 등)에 테두리 두께를 더함
        AdjustWindowRect(&rc, WS_OVERLAPPEDWINDOW, FALSE);  // 테두리 두께 계산
        // 윈도우 창의 위치(x,y)는 유지하고 크기만 계산된 값으로 바꿈
        SetWindowPos(hWnd, nullptr, 0, 0, rc.right - rc.left, rc.bottom - rc.top, SWP_NOMOVE | SWP_NOZORDER);
    }

    // 변경이 끝났으니 깃발(NeedsResize)을 내림
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // 2. DX11 Device & SwapChain 생성 // [2. DirectX 11 초기화 단계]
   ...
   // [4. HLSL 셰이더 소스 및 컴파일]
    const char* shaderSource = R"(
        ...
        PS_INPUT VS_Main(VS_INPUT input){
            PS_INPUT output;   
            output.pos = float4(input.pos, 1.0f);       // 3D를 4D로 확장 (w값=1.0f 필수)// 마지막 1.0f(w값)는 행렬 연산과 투영을 위해 필요함.
    ...
    // [중요]Ia (input Assembler) 설계도(InputLayout) 만들기: "시맨틱(POSITION, COLOR)은 CPU-GPU간 약속이다!" 
    D3D11_INPUT_ELEMENT_DESC layout[] = {
        { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
        { "COLOR", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0 },
    };
    g_pd3dDevice->CreateInputLayout(layout, 2, vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(), &g_pInputLayout); //1단계: IA(Input Assembler) - 조립 및 신분증 부착
    ...
    while (WM_QUIT != msg.message) {
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
            ...
            //  해상도 변경 깃발이 올라오면 리소스 재구성
            if (g_Config.NeedsResize) RebuildVideoResources(hWnd);


            // [렌더링]
            float clearColor[] = { 0.1f, 0.2f, 0.3f, 1.0f };// 배경색 설정
            g_pImmediateContext->ClearRenderTargetView(g_pRenderTargetView, clearColor);

            // 중요: 뷰포트는 매 프레임 바뀐 g_Config 기준으로 설정
            // 뷰포트(Viewport) 설정: 도화지 어디에 그릴지 결정
            D3D11_VIEWPORT vp = { 0.0f, 0.0f, (float)g_Config.Width, (float)g_Config.Height, 0.0f, 1.0f };
            g_pImmediateContext->RSSetViewports(1, &vp);            // 3단계 RS (Rasterizer) - 픽셀화
			g_pImmediateContext->OMSetRenderTargets(1, &g_pRenderTargetView, nullptr);  // 5단계 OM (Output Merger) - 최종 병합/출력 다 그린 그림을 g_pRenderTargetView(도화지)에 붙이는 단계

            UINT stride = sizeof(Vertex), offset = 0;
			g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer, &stride, &offset); // 1단계 IA(Input Assembler) - 조립 및 신분증 부착(실제 장착)
			g_pImmediateContext->IASetInputLayout(g_pInputLayout);          // 1단계 IA(Input Assembler) - 조립 및 신분증 부착
            g_pImmediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
            g_pImmediateContext->VSSetShader(g_pVertexShader, nullptr, 0);      //2단계 VS (Vertex Shader) - 정점 처리(가공) 미리 만들어둔 정점 로봇을 배치
			g_pImmediateContext->PSSetShader(g_pPixelShader, nullptr, 0);   //4단계 PS (Pixel Shader) - 픽셀 처리(채색) 미리 만들어둔 픽셀 로봇을 배치
