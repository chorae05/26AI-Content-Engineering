...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	// --- [1. 윈도우 생성 단계] ---  01_BLANKSCREEN+DRAWING과 거의 동일한 창 생성 코드
    ...
    // 2단계: "800x600 그림을 그리려면, 창 전체(테두리 포함)는 얼마나 커야 하니?"라고 물어봄
    AdjustWindowRect(&rc, WS_OVERLAPPEDWINDOW, FALSE);


	//윈도우 창 제작 주문서(실제 창 생성) ||  3단계: 계산된 창 크기(rc)를 적용해서 실제 창을 생성함
    // 
	//01_BLANKSCREEN+DRAWING과 거의 동일한 창 생성 코드
    HWND hWnd = CreateWindowW(L"DX11EngineClass", L"통합 해상도 관리 예제",  // 1. 사용할 설계도 이름 (아까 등록한 거랑 똑같아야 함!) 2. 창 제목 (제목 표시줄에 뜨는 글자)
        WS_OVERLAPPEDWINDOW,        // 3. 창 스타일 (최소화, 최대화 버튼 있는 기본 창)
        CW_USEDEFAULT, CW_USEDEFAULT, //4. 창이 뜰 위치 (x, y 좌표 - 윈도우가 알아서 정해줘!)
        rc.right - rc.left, rc.bottom - rc.top, //5. 창의 크기 (가로 800, 세로 600)
		nullptr, nullptr, hInstance, nullptr);      // 부모창 없음, 메뉴 없음, 신분증 전달

    ShowWindow(hWnd, nCmdShow);         //창을 모니터에 출력


	//01_BLANKSCREEN+DRAWING과 거의 동일한 창 생성 코드
    // --- [ DX11 초기화] --- 화면을 어떻게 교체할지 적어둔 주문서라고 생각하면 돼.
    DXGI_SWAP_CHAIN_DESC sd = {};
    sd.BufferCount = 1;
    // [중요] 스왑체인의 버퍼 크기는 곧 '백버퍼'의 물리 해상도임.
    // 중앙 설정값(g_Config)을 사용하여 데이터 일관성을 유지함.
    // 도화지(백버퍼)의 물리적인 픽셀 개수를 설정 (800x600)
    ...
    // GPU와 통신할 통로(Device)와 화면(SwapChain)을 생성함.  // 장치와 스왑체인(앞뒤 도화지 교체기) 생성
    D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, 0, nullptr, 0,
        D3D11_SDK_VERSION, &sd, &g_pSwapChain, &g_pd3dDevice, nullptr, &g_pImmediateContext);

    // 렌더 타겟 설정 (도화지 준비) // 백버퍼 텍스처를 가져와서 렌더 타겟 뷰(도화지 표면) 생성
    ...
    /*
     typedef struct D3D11_INPUT_ELEMENT_DESC {
         ...
         UINT                       AlignedByteOffset;    // 5. 오프셋 (시작 지점)각 정점 데이터 내에서(위치, 색상 등가 시작되는 바이트 위치를 지정함
    ...
    while (WM_QUIT != msg.message) {
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
            ...
            D3D11_VIEWPORT vp;
            vp.TopLeftX = 0;                                //뷰포트 시작 X 좌표 왼족 맨 위부터 시작 
            vp.TopLeftY = 0;                                // 뷰포트 시작 Y 좌표
            vp.Width = (float)g_Config.Width;               // 뷰포트 가로 폭     가로80만큼 채움
            vp.Height = (float)g_Config.Height;             // 뷰포트 세로 폭     세로 600만큼 채움
            vp.MinDepth = 0.0f;                             // 깊이 버퍼의 최소값 (0.0 ~ 1.0)   깊이 최소값
            vp.MaxDepth = 1.0f;                             // 깊이 버퍼의 최대값       깊이 최대값

            // RS(Rasterizer Stage): 점을 픽셀로 쪼개는 단계에 뷰포트 정보 전달
            g_pImmediateContext->RSSetViewports(1, &vp);
            // 최종 출력 대상 설정 (OM: Output Merger 단계) 출력 병합기는 픽셀 셰이더의 결과물과 기존 도화지의 내용을 합쳐 최종화면을 만드는 파이프라인의 마지막 단계임
